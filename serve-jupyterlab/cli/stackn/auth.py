...
global_vars = {"STACKN_CONFIG_PATH": "~/.scaleout", "STACKN_CONFIG_FILE": "stackn.json"}
...
def _check_flag_insecure(inp_conf):
    ...
    if (
        ...
        print(
            "Do you perhaps have a local STACKn deployment? If so, don't forget to use --insecure to make stackn API calls working."
        )
        return True
    else:
        return False
...
def _is_user_logged():
    ...
    if not stackn_conf_file:
        ...
        print(
            "Please use the command: 'stackn login -u <your-user> -p <your-user-password>' --url <your-studio-url>"
        )
        return False
    else:
        return True
    ...
    if stackn_config_full:
        try:
            ...
            if conf["STACKN_URL"] and not is_login:
                print(
                    "Failed to load config file from URL: {} not set up.".format(
                        conf["STACKN_URL"]
                    )
                )
                print(e)
...
def _set_current(conf):
    current = {"STACKN_URL": False, "STACKN_PROJECT": False, "STACKN_SECURE": "NOTSET"}
...
def get_config(inp_config=dict(), required=[], is_login=False, print_warnings=True):
    ...
    if "STACKN_SECURE" in current and (
        conf["STACKN_SECURE"] == "" or conf["STACKN_SECURE"] == None
    ):
