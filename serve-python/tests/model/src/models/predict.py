...
def model_predict(inp, model=[]):
    ...
    tokenizer = BertTokenizer.from_pretrained(
        "models/src/models/vocab_swebert.txt", do_lower_case=False
    )
    ...
    bert_word_piece_tokenizer = BertWordPieceTokenizer(
        "models/src/models/vocab_swebert.txt", lowercase=True, strip_accents=False
    )
    ...
    predicted_index_top5 = torch.argsort(predictions[0, masked_index], descending=True)[
        :5
    ]
